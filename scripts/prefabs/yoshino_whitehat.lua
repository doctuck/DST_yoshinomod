...
local function onequip(inst, owner)
    ...
    inst.components.equippable.spireng = -2 / 60             --每分钟减少 2 灵力(通过挂载实现)
...
local function fn()
    ...
    inst.AnimState:SetBank("yoshino_whitehat")  --地上动画，scml文件中的动画集合名
    inst.AnimState:SetBuild("yoshino_whitehat") --地上动画素材包，zip文件名
    inst.AnimState:PlayAnimation("idle")        --默认播放的动画，需要指定scml文件里的动画集合下的具体动画名

    inst:AddTag("warable")                    --该物品属于衣服
    inst:AddTag("yoshino_whitehat")           --添加物品标签
    inst:AddTag("goggles")                    --防风镜效果
    ...
    inst.components.inventoryitem.imagename = "yoshino_whitehat"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/yoshino_whitehat.xml"
    ...
    inst.components.fueled:InitializeFuelLevel(TUNING.AUTOSAVE_INTERVAL*8) --持续时间：8天
